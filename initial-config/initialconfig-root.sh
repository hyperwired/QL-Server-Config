...
# apt-get update; apt-get -y upgrade; apt-get -y dist-upgrade; apt-get -y purge $(dpkg -l | awk '/^rc/ { print $2 }'); apt-get -y autoremove; reboot

# Set the time-zone
dpkg-reconfigure tzdata

# Install dependencies (during sid-repo activation, may need to install binutils if C compiler can't compile anything)
apt-get -y install lib32gcc1 sudo curl nano samba build-essential python-dev python-setuptools python3 unzip dos2unix cron mailutils wget lib32z1 lib32stdc++6 libc6 git python-pip
