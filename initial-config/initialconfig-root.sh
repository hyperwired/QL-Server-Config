...
sudo bash -c "echo \"deb http://ftp.debian.org/debian sid main\" >> /etc/apt/sources.list"; apt-get update; apt-get install python3.5 python3.5-dev; sudo sed -i '$ d' /etc/apt/sources.list; apt-get update
easy_install supervisor pyzmq hiredis
