...
class mybalance(minqlx.Plugin):
    ...
    def cmd_elo_type(self, player, msg, channel):
        if len(msg) < 2:
            ...
            if not self.db.has_permission(player, 3):
                player.tell("^4You don't have the required permission (3) to perform this action. ")
            ...
            channel.reply("^7Switched to ^4{}^7 rankings.".format(msg[1].upper()))
            return
    ...
    def cmd_min_elo(self, player, msg, channel):
        if len(msg) < 2:
            channel.reply("^7The minimum skill rating required for this server is: ^4{}^7.".format(self.ELO_MIN))
        elif len(msg) < 3:
            try:
                new_elo = int(msg[1])
                assert new_elo >= 0
            ...
            channel.reply("^7The server minimum skill rating has been temporarily set to: ^4{}^7.".format(new_elo))
        else:
            return minqlx.RET_USAGE

    def cmd_max_elo(self, player, msg, channel):
        if len(msg) < 2:
            channel.reply("^7The maximum skill rating set for this server is: ^4{}^7.".format(self.ELO_MAX))
        elif len(msg) < 3:
            try:
                new_elo = int(msg[1])
                assert new_elo >= 0
            ...
            channel.reply("^7The server maximum skill ratings has been temporarily set to: ^4{}^7.".format(new_elo))
        else:
            return minqlx.RET_USAGE

    def cmd_elo_limit(self, player, msg, channel):
        if int(self.get_cvar('qlx_elo_block_connecters')):
            self.msg("^7The server will block players upon connection who fall outside [^4{}^7-^4{}^7].".format(self.ELO_MIN, self.ELO_MAX))
        elif int(self.get_cvar('qlx_elo_kick')):
            self.msg("^7The server will kick players who fall outside [^4{}^7-^4{}^7].".format(self.ELO_MIN, self.ELO_MAX))
        else:
            self.msg("^7Players who don't have a skill rating between ^4{} ^7and ^4{} ^7are only allowed to spec.".format(self.ELO_MIN, self.ELO_MAX))
    ...
    def cmd_elo_kicked(self, player, msg, channel):
        ...
        for sid in self.kicked:
            ...
            m = "^7{}: ^4{}^7 - ^4{}^7 - ^4{}".format(n, sid, elo, name)
            channel.reply(m)
            n += 1
    ...
    def cmd_rem_kicked(self, player, msg, channel):
        ...
        for sid in self.kicked.copy():
            if not n:
                ...
                n -= 1
        channel.reply("^7Successfully removed ^4{}^7 (glicko {}) from the list.".format(name, elo))
    ...
    def cmd_nokick(self, player, msg, channel):
        ...
        if not self.kickthreads:
                player.tell("^4Psst^7: There are no people being kicked right now.")
                return minqlx.RET_STOP_ALL
        ...
        if len(msg) < 2:
            ...
            player.tell("^4Psst^7: did you mean ^4{}^7?".format("^7 or ^4".join(_names)))
            return minqlx.RET_STOP_ALL
        ...
        # If a search term, name, was given
            ...
            if not match_threads:
                player.tell("^4Psst^7: no players matched '^4{}^7'?".format(msg[1]))
                return minqlx.RET_STOP_ALL
        ...
        try:
            ...
            with open (abs_file_path, "r") as file:
                for line in file:
                    sid, name = line.split(" ")
                    if int(sid) == add_sid:
                        player.tell("^4Psst: ^7This ID is already in the exception list under name ^4{}^7!".format(name))
                        return minqlx.RET_STOP_ALL
            ...
            player.tell("^4Psst: IOError: ^7{}".format(e))

        except ValueError as e:
            return minqlx.RET_USAGE

        except Exception as e:
            player.tell("^4Psst: ^1Error: ^7{}".format(e))
        return minqlx.RET_STOP_ALL
    ...
    def cmd_help_load_exceptions(self, player, msg, channel):
        try:
            ...
            with open (abs_file_path, "r") as file:
                ...
                for line in file:
                    ...
                    try:
                        ...
                        if player:
                            player.tell("^4Psst: ^2Loaded: ^7{} ({})".format(sid, name.strip('\n\r\t')))
            ...
            try:
                ...
                minqlx.CHAT_CHANNEL.reply("^4mybalance plugin^7: No exception list found, so I made one myself.")
            except:
    ...
    def cmd_last_action(self, player, msg, channel):
        if len(msg) < 2:
            return channel.reply("^7The current action when teams are uneven is: ^4{}^7.".format(self.last_action))

        if msg[1] not in ["slay", "spec", "ignore"]:
            return minqlx.RET_USAGE

        self.last_action = msg[1]
        channel.reply("^7Action has been succesfully changed to: ^4{}^7.".format(msg[1]))
    ...
    def handle_round_count(self, round_number):
        ...
        if abs(diff) >= 1:
            if is_even(diff):
                ...
                self.msg("^4Uneven teams detected!^7 At round start i'll move {} to {}".format(n, to))
            else:
                m = 'lowest player' if n == 1 else '{} lowest players'.format(n)
                m = " and move the {} to {}".format(m, to) if n else ''
                self.msg("^4Uneven teams detected!^7 Server will auto spec {}{}.".format(last.name, m))
    ...
    def callback_elo(self, player, elo = 0, games=0):
        ...
        if key in self.db:
            ...
            elos.append("^7local {} glicko: ^4{}".format(self.game.type_short.upper(), dbelo))
        #if elo and games:
        elos.append("^7qlstats.net {} glicko: ^4{} ({} games)".format(self.game.type_short.upper(), elo, games))

        if elos: minqlx.CHAT_CHANNEL.reply("^4{}".format(m) + " ^7, ".join(elos) + "^7.")
        ...
        class kickThread(threading.Thread):
            ...
            def try_mess(self):
                ...
                if self.plugin.get_cvar("qlx_elo_kick") == "1":
                    kickmsg = "so you'll be ^4kicked ^7shortly..."
                else:
                    kickmsg = "but you are free to keep watching."
    ...
    def find_by_name_or_id(self, player, target):
        ...
        def list_alternatives(players, indent=2):
            player.tell("A total of ^4{}^7 players matched for {}:".format(len(players),target))
            out = ""
            for p in players:
                out += " " * indent
                out += "{}^4:^7 {}\n".format(p.id, p.name)
