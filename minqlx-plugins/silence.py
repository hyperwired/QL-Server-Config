...
class silence(minqlx.Plugin):
    ...
    def handle_player_loaded(self, player):
        ...
        player.mute()
        if reason:
            player.tell("You are muted on this server until ^4{}^7: {}".format(expires, reason))
        else:
            player.tell("You are muted on this server until ^4{}^7.".format(expires))
    ...
    def cmd_silence(self, player, msg, channel):
        ...
        if self.db.has_permission(ident, 2):
            channel.reply("^4{}^7 has permission level 2 or more and cannot be silenced.".format(name))
            return
        ...
        if r:
            ...
            if target_player:
                ...
                try:
                    ...
                    pass
            channel.reply("^4{} ^7has been silenced. Silence expires on ^4{}^7.".format(name, expires))
    ...
    def cmd_unsilence(self, player, msg, channel):
        ...
        if not silences:
            channel.reply("^7 No active silences on ^4{}^7 found.".format(name))
        else:
            db = self.db.pipeline()
            ...
            channel.reply("^4{}^7 has been unsilenced.".format(name))
    ...
    def cmd_checksilence(self, player, msg, channel):
        ...
        channel.reply("^4{} ^7is not silenced.".format(name))
