...
class essentials(minqlx.Plugin):
    ...
    def cmd_disconnects(self, player, msg, channel):
        if len(self.recent_dcs) == 0:
            ...
            player.tell("The most recent ^4{}^7 player disconnects:".format(len(self.recent_dcs)))
            for x in self.recent_dcs:
                p, t = x
                player.tell("  {} ({}): ^4{}^7 seconds ago".format(p.name, p.steam_id, round(time.time() - t)))
    ...
    def cmd_enable_sounds(self, player, msg, channel):
        ...
        if flag:
            player.tell("Sounds have been disabled. Use ^4{}sounds^7 to enable them again."
                .format(self.get_cvar("qlx_commandPrefix")))
        else:
            player.tell("Sounds have been enabled. Use ^4{}sounds^7 to disable them again."
                .format(self.get_cvar("qlx_commandPrefix")))
    ...
    def cmd_sound(self, player, msg, channel):
        ...
        if not self.db.get_flag(player, "essentials:sounds_enabled", default=True):
            player.tell("Sounds are disabled. Use ^4{}sounds^7 to enable them again."
                .format(self.get_cvar("qlx_commandPrefix")))
            return minqlx.RET_STOP_ALL
