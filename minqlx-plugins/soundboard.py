...
class soundboard(minqlx.Plugin):
    ...
    def play_new_game_sound(self, *args, **kwargs):
        self.play(NEW_GAME_SOUND[0], NEW_GAME_SOUND[1])
        
    def play_game_countdown_sound(self, *args, **kwargs):
        self.play(GAME_COUNTDOWN_SOUND[0], GAME_COUNTDOWN_SOUND[1])

    def play_game_start_sound(self, *args, **kwargs):
        self.play(GAME_START_SOUND[0], GAME_START_SOUND[1])
        
    def play_game_end_sound(self, *args, **kwargs):
        self.play(GAME_END_SOUND[0], GAME_END_SOUND[1])
        
    def play_round_countdown_sound(self, *args, **kwargs):
        self.play(ROUND_COUNTDOWN_SOUND[0], ROUND_COUNTDOWN_SOUND[1])
        
    def play_round_start_sound(self, *args, **kwargs):
        self.play(ROUND_START_SOUND[0], ROUND_START_SOUND[1])
        
    def play_round_end_sound(self, *args, **kwargs):
        self.play(ROUND_END_SOUND[0], ROUND_END_SOUND[1])
        
    def play_player_connect_sound(self, *args, **kwargs):
        self.play(PLAYER_CONNECT_SOUND[0], PLAYER_CONNECT_SOUND[1])
        
    def play_player_loaded_sound(self, *args, **kwargs):
        self.play(PLAYER_LOADED_SOUND[0], PLAYER_LOADED_SOUND[1])
        
    def play_player_disconnect_sound(self, *args, **kwargs):
        self.play(PLAYER_DISCONNECT_SOUND[0], PLAYER_DISCONNECT_SOUND[1])
        
    def play_player_voted_sound(self, player, yes):
        if yes:
            self.play(PLAYER_VOTED_YES_SOUND[0], PLAYER_VOTED_YES_SOUND[1], player)
        else:
            self.play(PLAYER_VOTED_NO_SOUND[0], PLAYER_VOTED_NO_SOUND[1], player)
        
    def play_vote_called_sound(self, *args, **kwargs):
        self.play(VOTE_CALLED_SOUND[0], VOTE_CALLED_SOUND[1])
        
    def play_vote_ended_sound(self, votes, vote, args, passed):
        if passed:
            self.play(VOTE_PASSED_SOUND[0], VOTE_PASSED_SOUND[1])
        else:
            self.play(VOTE_FAILED_SOUND[0], VOTE_FAILED_SOUND[1])

    def load_workshop_items(self, *args, **kwargs):
        self.game.workshop_items += [645733939]

    def play(self, audiofile, isMusic=False, player=None):
        if not audiofile: return
        if isMusic:
            self.play_music("soundboard/" + audiofile, player)
        else:
            self.play_sound("soundboard/" + audiofile, player)
